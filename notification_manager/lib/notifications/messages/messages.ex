defmodule Notifications.Messages do
  @moduledoc """
  ...
  def filter_by_user_id(query, user_id) do
    ...
    if user_id == nil do
      ...
      query |> where([notification], notification.user_id == ^user_id)
    end
  end

  def query(notifications) do
  ...
  def filter_by_start_date(query, start_date) do
    if start_date == nil do
      ...
      query |> where([notification], notification.inserted_at>=^start_date)
    end
  end
  ...
  def filter_by_end_date(query, end_date) do
    if end_date == nil do
      ...
      query |> where([notification], notification.inserted_at<=^end_date)
    end
  end
